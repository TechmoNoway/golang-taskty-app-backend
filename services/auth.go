...
import (
	...
	"github.com/TechmoNoway/golang-taskty-app-backend/models"
	"github.com/TechmoNoway/golang-taskty-app-backend/utils"
...
func (s *AuthService) Register(ctx context.Context, registerData *models.AuthCredentials) (string, *models.User, error) {
	...
	_, err := s.repository.GetUser(ctx, "email = ?", registerData.Email)
	if !errors.Is(err, gorm.ErrRecordNotFound) {
